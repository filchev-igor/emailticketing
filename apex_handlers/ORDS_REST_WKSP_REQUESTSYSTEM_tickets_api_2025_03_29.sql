...
-- Schema: WKSP_REQUESTSYSTEM  Date: Sat Mar 29 09:57:03 2025 
--
...
BEGIN
    ...
    -- 2) Continue with normal logic if autho' || 'rized
    ...
    IF l_json_parsed THEN
     ...
     ' || '   l_body := APEX_JSON.get_clob(p_path => ''body'');
        ...
        -- Insert into tickets
        INSERT I' || 'NTO tickets (email_id, sender_id, subject, body, status, creation_date, update_date)
        ...
        APEX_JSON.write(''message'', ''Ticket created success' || 'fully'');
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        APEX_JSON.open_object;
        APEX_JSON.write(''status'', ''error'');
        APEX_JSON.write(''m' || 'essage'', SQLERRM);
        APEX_JSON.close_object;
END;
');
